...
{% code overflow="wrap" %}
```powershell
$c=[regex]::Matches((Get-Content .\chain.pem -Raw),'(?s)-----BEGIN CERTIFICATE-----.?-----END CERTIFICATE-----').Value | % { [Security.Cryptography.X509Certificates.X509Certificate2]::new([Text.Encoding]::ASCII.GetBytes($)) }; $ord=[Collections.ArrayList]@(); $seen=@{}; $cur=$c | ? { $.Subject -eq $.Issuer } | Select -First 1; while($cur){ [void]$ord.Add($cur); $seen[$cur.Thumbprint]=1; $cur=$c | ? { $.Issuer -eq $ord[-1].Subject -and -not $seen[$.Thumbprint] } | Select -First 1 }; $c | ? { -not $seen[$.Thumbprint] } | % { [void]$ord.Add($) }; $i=0; $ord | % { $cn=($.GetNameInfo('SimpleName',$false) -replace '[\/:?"<>|]',''); $f=".{0:d2}-{1}.pem" -f $i++,$cn; Set-Content $f ("-----BEGIN CERTIFICATE-----n"+[Convert]::ToBase64String($_.RawData,'InsertLineBreaks')+"n-----END CERTIFICATE-----") -Encoding ascii; "$f subject=$($.Subject) issuer=$($_.Issuer)" }
